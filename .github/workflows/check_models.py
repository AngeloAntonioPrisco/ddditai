...
def fetch_latest_models_and_results():
    ...
    if not training_folders:
        raise FileNotFoundError("Nessuna cartella di training trovata.")

    latest_folder = sorted(training_folders, reverse=True)[0]
    print(f"Cartella di training più recente trovata: {latest_folder}")

    # Recupera i modelli
    model_blobs = [b for b in all_blobs if b.startswith(f"{TRAINING_PREFIX}{latest_folder}/models/") and b.endswith(".onnx")]
    if not model_blobs:
        raise FileNotFoundError(f"Nessun modello ONNX trovato in {latest_folder}/models")
    ...
    for blob_name in model_blobs:
        ...
        print(f"Modello scaricato: {local_path}")

    # Recupera i CSV dei risultati
    result_blobs = [b for b in all_blobs if b.startswith(f"{TRAINING_PREFIX}{latest_folder}/results/") and b.endswith(".csv")]
    if not result_blobs:
        raise FileNotFoundError(f"Nessun CSV risultati trovato in {latest_folder}/results")
    ...
    for blob_name in result_blobs:
        ...
        model_results[result_name] = df.iloc[0].to_dict()  # prima riga con metriche
        print(f"Risultati scaricati: {local_csv_path}")

    # Crea lista di tuple (modello, risultati)
    model_result_pairs = []
    for model_path in local_models:
        ...
        if model_name in model_results:
            ...
            print(f"Attenzione: nessun CSV risultato trovato per {model_name}")
